...
function MenuGarage()
    ...
    for veh, label in pairs(authorizedVehicles) do
        vehicleMenu[#vehicleMenu + 1] = {
            ...
            txt = "",
            params = {
                event = "traumateam:client:TakeOutVehicle",
                args = {
                    vehicle = veh
                }
            }
...
RegisterNetEvent('traumateam:client:TakeOutVehicle', function(data)
...
RegisterNetEvent('QBCore:Client:OnJobUpdate', function(JobInfo)
    PlayerJob = JobInfo
    if PlayerJob.name == 'traumateam' then
...
RegisterNetEvent('QBCore:Client:OnPlayerLoaded', function()
    ...
    CreateThread(function()
        ...
        QBCore.Functions.GetPlayerData(function(PlayerData)
            ...
            if PlayerJob.name == 'traumateam' and onDuty then
                TriggerServerEvent("hospital:server:AddDoctor", PlayerJob.name)
            end
        end)
    end)
...
RegisterNetEvent('QBCore:Client:SetDuty', function(duty)
    if PlayerJob.name == 'traumateam' and duty ~= onDuty then
        if duty then
            TriggerServerEvent("hospital:server:AddDoctor", PlayerJob.name)
        else
            TriggerServerEvent("hospital:server:RemoveDoctor", PlayerJob.name)
        ...
        while check do
            if IsControlJustPressed(0, 38) then
                ...
                elseif variable == "stash" then
                    TriggerEvent('tsrp-traumateamjob:stash')
                elseif variable == "armory" then
                    TriggerEvent('tsrp-traumateamjob:armory')
                elseif variable == "storeheli" then
                    TriggerEvent('tsrp-traumateamjob:storeheli')
                elseif variable == "takeheli" then
                    TriggerEvent('tsrp-traumateamjob:pullheli')
                elseif variable == "roof" then
                    TriggerEvent('tsrp-traumateamjob:elevator_main')
                elseif variable == "main" then
                    TriggerEvent('tsrp-traumateamjob:elevator_roof')
                end
            end
            Wait(1)
        end
...
RegisterNetEvent('tsrp-traumateamjob:stash', function()
    if onDuty then
        TriggerServerEvent("inventory:server:OpenInventory", "stash",
            "traumateamstash_" .. QBCore.Functions.GetPlayerData().citizenid)
        TriggerEvent("inventory:client:SetCurrentStash", "traumateamstash_" .. QBCore.Functions.GetPlayerData().citizenid)
    end
end)

RegisterNetEvent('tsrp-traumateamjob:armory', function()
    if onDuty then
            ...
            if IsControlJustPressed(0, 38) then
                ...
                if IsPedInAnyVehicle(ped, false) then
                    ...
                    QBCore.Functions.TriggerCallback('QBCore:Server:SpawnVehicle', function(netId)
                        ...
                        SetEntityHeading(veh, coords.w)
                        SetVehicleLivery(veh, 1) -- traumateam Livery
                        exports['LegacyFuel']:SetFuel(veh, 100.0)
                        TaskWarpPedIntoVehicle(PlayerPedId(), veh, -1)
                        TriggerEvent("vehiclekeys:client:SetOwner", QBCore.Functions.GetPlate(veh))
                        SetVehicleEngineOn(veh, true, true)
...
RegisterNetEvent('tsrp-traumateamjob:elevator_roof', function()
    local ped = PlayerPedId()
...
RegisterNetEvent('tsrp-traumateamjob:elevator_main', function()
    local ped = PlayerPedId()
...
CreateThread(function()
    for k, v in pairs(Config.Locations["vehicle"]) do
        ...
        boxZone:onPlayerInOut(function(isPointInside)
            if isPointInside and PlayerJob.name == "traumateam" and onDuty then
...
if Config.UseTarget then
        ...
        for k, v in pairs(Config.Locations["stash"]) do
            exports['qb-target']:AddBoxZone("stash" .. k, vector3(v.x, v.y, v.z), 1, 1, {
                ...
                options = {
                    {
                        type = "client",
                        event = "tsrp-traumateamjob:stash",
                        icon = "fa fa-hand",
                        label = "Open Stash",
                        job = "traumateam"
                    }
        ...
        for k, v in pairs(Config.Locations["armory"]) do
            exports['qb-target']:AddBoxZone("armory" .. k, vector3(v.x, v.y, v.z), 1, 1, {
                ...
                options = {
                    {
                        type = "client",
                        event = "tsrp-traumateamjob:armory",
                        icon = "fa fa-hand",
                        label = "Open Armory",
                        job = "traumateam"
                    }
        ...
        for k, v in pairs(Config.Locations["roof"]) do
            exports['qb-target']:AddBoxZone("roof" .. k, vector3(v.x, v.y, v.z), 2, 2, {
                ...
                options = {
                    {
                        type = "client",
                        event = "tsrp-traumateamjob:elevator_roof",
        ...
        for k, v in pairs(Config.Locations["main"]) do
            exports['qb-target']:AddBoxZone("main" .. k, vector3(v.x, v.y, v.z), 1.5, 1.5, {
                ...
                options = {
                    {
                        type = "client",
                        event = "tsrp-traumateamjob:elevator_main",
                        icon = "fas fa-hand-point-up",
                        label = "Take Elevator",
                        job = "traumateam"
                    },
    ...
    CreateThread(function()
        ...
        signCombo:onPlayerInOut(function(isPointInside)
            if isPointInside and PlayerJob.name == "traumateam" then
                if not onDuty then
