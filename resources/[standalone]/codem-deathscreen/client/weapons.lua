...
RegisterNetEvent('codem-deathscreen:giveup')
AddEventHandler('codem-deathscreen:giveup', function()
  playerkill = false
  timeFinish = false 

  if Config.Framework == 'esx' then
    TriggerEvent('codem-client:Revive')
...
RegisterNetEvent('codem-deathscreen:waitDoctor')
AddEventHandler('codem-deathscreen:waitDoctor', function()
  if Config.Framework == 'esx' then 
    if found then 
      found = false
      SendNUIMessage({
        action = 'NOTIFY',
        message = Config.Locale['HOSPITAL']
      })
      TriggerEvent('esx_ambulancejob:signal:codem')
      Citizen.Wait(10000)
      found = true
    else
      SendNUIMessage({
...
WeaponNames = {
    ...
    [tostring(GetHashKey('WEAPON_RUN_OVER_BY_CAR'))] = 'Araç ile ezdi.',
...
function getKillerLabel(DeathCauseHash)
