...
function CheckDirtyMoney(source, amount, remove)
    ...
    elseif Config.Framework == "qbcore" then
        ...
        if total_worth > amount and markedbills and markedbills.amount > 0 then
            local remaining_worth = total_worth - amount
            local last_markedbills_slot = nil
            local last_markedbills_worth = nil

            for itemkey, item in pairs(xPlayer.PlayerData.items) do
                if item.name == 'markedbills' and type(item.info) ~= 'string' then
                    if not last_markedbills_worth or item.slot > last_markedbills_slot then
                        last_markedbills_slot = item.slot
                        last_markedbills_worth = tonumber(item.info.worth)
                    end
                end
            end
            info = {}
            if last_markedbills_slot then
                if remove then
                    info = {}
                    info.worth = tonumber(remaining_worth)
                    xPlayer.Functions.RemoveItem('markedbills', 1, nil, last_markedbills_slot)
                    xPlayer.Functions.AddItem('markedbills', 1, nil, info)
                    AddItem(cutItems.AddItems.item, cutAmount, src)
                end
                return true
            else
