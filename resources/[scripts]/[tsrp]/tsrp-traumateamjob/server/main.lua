...
RegisterNetEvent('hospital:server:SendToBed', function(bedId, isRevive)
	...
	Player.Functions.RemoveMoney("bank", Config.BillCost , "respawned-at-hospital")
		exports['qb-management']:AddMoney("traumateam", Config.BillCost)
...
RegisterNetEvent('hospital:server:traumateamAlert', function(text)
    local src = source
    local ped = GetPlayerPed(src)
    local coords = GetEntityCoords(ped)
    local players = QBCore.Functions.GetQBPlayers()
    for _, v in pairs(players) do
        if v.PlayerData.job.name == 'traumateam' and v.PlayerData.job.onduty then
            TriggerClientEvent('hospital:client:traumateamAlert', v.PlayerData.source, coords, text)
        end
    end
end)
...
RegisterNetEvent('hospital:server:TreatWounds', function(playerId)
	...
	if Patient then
		if Player.PlayerData.job.name =="traumateam" then
...
RegisterNetEvent('hospital:server:RevivePlayer', function(playerId, isOldMan)
	...
	if Patient then
		if Player.PlayerData.job.name == "traumateam" or QBCore.Functions.HasItem(src, "firstaid", 1) then
			if oldMan then
			...
			TriggerEvent('qb-log:server:CreateLog', 'traumateamjob', 'Player Banned', 'red', string.format('%s was banned by %s for %s', GetPlayerName(src), 'tsrp-traumateamjob', "Trying to revive theirselves or other players"), true)
...
RegisterNetEvent('hospital:server:SendDoctorAlert', function()
    ...
    if not doctorCalled then
        ...
        for _, v in pairs(players) do
            if v.PlayerData.job.name == 'traumateam' and v.PlayerData.job.onduty then
                TriggerClientEvent('QBCore:Notify', v.PlayerData.source, Lang:t('info.dr_needed'), 'traumateam')
            end
        end
...
QBCore.Commands.Add('911e', Lang:t('info.ems_report'), {{name = 'message', help = Lang:t('info.message_sent')}}, false, function(source, args)
    ...
    for _, v in pairs(players) do
        if v.PlayerData.job.name == 'traumateam' and v.PlayerData.job.onduty then
            TriggerClientEvent('hospital:client:traumateamAlert', v.PlayerData.source, coords, message)
        end
    end
end)
